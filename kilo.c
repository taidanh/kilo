        ...
        for (j = 0; j < len; j++) {
            if (iscntrl(c[j])) {
                ...
                if (current_color != -1) {
                    char buf[16];
                    int clen;
                    // if (current_color == editorSyntaxToColor(HL_KEYWORD1) ||
                    //     current_color == editorSyntaxToColor(HL_KEYWORD2)) {
                    if (current_color == 32 || current_color == 33) {
                        clen = snprintf(buf, sizeof(buf), "\x1b[1;%dm", current_color);
                    } else {
